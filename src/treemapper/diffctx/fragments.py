...
_INDENT_EXTENSIONS = {".py", ".pyw", ".pyi"} | _YAML_EXTENSIONS
...
def _build_covered_set(covered: list[tuple[int, int]]) -> set[int]:
    result: set[int] = set()
...
def _find_uncovered_lines(total_lines: int, covered_set: set[int]) -> list[int]:
    return [ln for ln in range(1, total_lines + 1) if ln not in covered_set]


def _group_into_gaps(uncovered_lines: list[int]) -> list[tuple[int, int]]:
...
def _trim_blank_lines(lines: list[str], start: int, end: int) -> tuple[int, int]:
...
def _create_gap_fragment(path: Path, lines: list[str], start: int, end: int) -> Fragment | None:
...
def _create_code_gap_fragments(path: Path, lines: list[str], covered: list[tuple[int, int]]) -> list[Fragment]:
    if not lines:
        return []

    covered_set = _build_covered_set(covered)
    uncovered_lines = _find_uncovered_lines(len(lines), covered_set)
    if not uncovered_lines:
        return []

    gaps = _group_into_gaps(uncovered_lines)
    fragments: list[Fragment] = []

    for start, end in gaps:
        start, end = _trim_blank_lines(lines, start, end)
        frag = _create_gap_fragment(path, lines, start, end)
        if frag:
...
class TreeSitterStrategy:
    ...
    def fragment(self, path: Path, content: str) -> list[Fragment]:
        ...
        gap_frags = _create_code_gap_fragments(path, lines, list(covered))
...
class PythonAstStrategy:
    ...
    def fragment(self, path: Path, content: str) -> list[Fragment]:
        ...
        fragments.extend(_create_code_gap_fragments(path, lines, covered))
        return fragments
...
class ConfigStrategy:
    ...
    def can_handle(self, path: Path, _content: str) -> bool:
        return path.suffix.lower() in _YAML_EXTENSIONS | {".json", ".toml"}
