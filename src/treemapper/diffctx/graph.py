...
def _build_python_semantic_edges(fragments: list[Fragment]) -> dict[tuple[FragmentId, FragmentId], float]:
    ...
    # Cache analysis results to avoid double parsing
    ...
    for f in py_frags:
        ...
        call_names = set(filter_idents(info.calls, min_len=3))
        ref_names = set(filter_idents(info.references, min_len=3))
        type_names = set(filter_idents(info.type_refs, min_len=3))
        ...
        for name in call_names:
            for dst in name_to_defs.get(name, []):
                if dst == f.id:
                    continue
                edges[(f.id, dst)] = max(edges.get((f.id, dst), 0.0), _CALL_WEIGHT)
                edges[(dst, f.id)] = max(edges.get((dst, f.id), 0.0), _CALL_WEIGHT * _BACKWARD_WEIGHT_FACTOR)
        ...
        for name in ref_names:
            ...
            for dst in name_to_defs.get(name, []):
                if dst == f.id:
                    continue
                edges[(f.id, dst)] = max(edges.get((f.id, dst), 0.0), _SYMBOL_REF_WEIGHT)
                edges[(dst, f.id)] = max(edges.get((dst, f.id), 0.0), _SYMBOL_REF_WEIGHT * _BACKWARD_WEIGHT_FACTOR)
        ...
        for name in type_names:
            ...
            for dst in name_to_defs.get(name, []):
                if dst == f.id:
                    continue
                edges[(f.id, dst)] = max(edges.get((f.id, dst), 0.0), _TYPE_REF_WEIGHT)
                edges[(dst, f.id)] = max(edges.get((dst, f.id), 0.0), _TYPE_REF_WEIGHT * _BACKWARD_WEIGHT_FACTOR)
    ...
    if imported.startswith(".") and source_path:
        ...
        if base_module:
            ...
            if dots <= len(parts):
                ...
                if resolved in module_to_frags:
                    return module_to_frags[resolved]
...
def _is_test_file(path: Path) -> bool:
    ...
    if suffix in (".js", ".ts", ".jsx", ".tsx"):
        if ".test." in name or ".spec." in name:
            return True
        if "__tests__" in path_str:
            return True

    # Rust
    if suffix == ".rs":
        if "/tests/" in path_str or name == "tests.rs":
            return True

    # Java/Kotlin
    if suffix in (".java", ".kt"):
        if name.endswith("test.java") or name.endswith("test.kt"):
            return True
...
def _build_lexical_edges_sparse(fragments: list[Fragment]) -> dict[tuple[FragmentId, FragmentId], float]:
    ...
    tf_idf_vectors: dict[FragmentId, dict[str, float]] = {}
    norms: dict[FragmentId, float] = {}
    ...
    # Map fragment IDs to paths for language-specific weights
    id_to_path: dict[FragmentId, Path] = {frag.id: frag.path for frag in fragments}
...
def _build_config_code_edges(fragments: list[Fragment]) -> dict[tuple[FragmentId, FragmentId], float]:
    ...
    for cfg in config_frags:
        ...
        if suffix in (".yaml", ".yml"):
            for match in _CONFIG_KEY_RE.finditer(cfg.content):
                config_keys.add(match.group(1).lower())
        elif suffix == ".json":
            for match in re.finditer(r'"([a-zA-Z_][a-zA-Z0-9_-]*)"\s*:', cfg.content):
                config_keys.add(match.group(1).lower())
        elif suffix == ".toml":
            for match in _TOML_KEY_RE.finditer(cfg.content):
                config_keys.add(match.group(1).lower())
            for match in _TOML_SECTION_RE.finditer(cfg.content):
                config_keys.add(match.group(1).lower())
        elif suffix == ".ini":
            for match in re.finditer(r"^\s*([a-zA-Z_][a-zA-Z0-9_-]*)\s*=", cfg.content, re.MULTILINE):
                config_keys.add(match.group(1).lower())
        elif suffix == ".env":
            for match in re.finditer(r"^([A-Z_][A-Z0-9_]*)\s*=", cfg.content, re.MULTILINE):
                config_keys.add(match.group(1).lower())
    ...
    for cfg in config_frags:
        keys = config_keys_by_frag.get(cfg.id, set())
        for key in keys:
            matching_code_frags = key_to_code_frags.get(key, [])
            if not matching_code_frags:
                continue

            # Diminishing returns: weight decreases with more matches
            match_count = len(matching_code_frags)
            adjusted_weight = _CONFIG_CODE_WEIGHT * min(1.0, 3.0 / match_count)

            for code_frag_id in matching_code_frags:
                edges[(cfg.id, code_frag_id)] = max(edges.get((cfg.id, code_frag_id), 0.0), adjusted_weight)
                edges[(code_frag_id, cfg.id)] = max(
                    edges.get((code_frag_id, cfg.id), 0.0), adjusted_weight * _BACKWARD_WEIGHT_FACTOR
                )
