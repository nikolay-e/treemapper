...
static DBT_BUILTINS: Lazy<FxHashSet<&str>> = Lazy::new(|| {
    ["ref", "source", "config", "set", "if", "for", "endif", "endfor", "else", "elif",
     "block", "endblock", "macro", "endmacro", "do", "call", "filter"]
        .iter().copied().collect()
});

fn extract_refs(content: &str) -> FxHashSet<String> {
    REF_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}

fn extract_sources(content: &str) -> FxHashSet<String> {
    SOURCE_RE.captures_iter(content).map(|c| c[2].to_string()).collect()
}

fn extract_macro_calls(content: &str) -> FxHashSet<String> {
    MACRO_CALL_RE.captures_iter(content)
        .map(|c| c[1].to_string())
...
fn extract_macro_defs(content: &str) -> FxHashSet<String> {
    MACRO_DEF_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}
...
impl EdgeBuilder for DbtEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        ...
        for f in &frags {
            for name in extract_macro_defs(&f.content) {
                macro_to_frags.entry(name.to_lowercase()).or_default().push(f.id.clone());
            }
        }
        ...
        for f in &frags {
            ...
            for mc in extract_macro_calls(&f.content) {
                if let Some(targets) = macro_to_frags.get(&mc.to_lowercase()) {
                    for t in targets { if t != &f.id { add_edge(&mut edges, &f.id, t, macro_w, macro_rev); } }
                }
            }
        }
        edges
    ...
    fn discover_related_files(
        ...
        for f in changed {
            if !is_sql_file(f) { continue; }
            if let Some(content) = base::read_file_cached(f, file_cache) {
                if !is_dbt_file(&content) { continue; }
