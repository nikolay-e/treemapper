...
use super::super::base::{self, add_edge, EdgeBuilder, FragmentIndex, discover_files_by_refs, link_by_path_match};
use super::super::EdgeDict;

static BAZEL_NAMES: Lazy<FxHashSet<&str>> =
    Lazy::new(|| ["BUILD", "BUILD.bazel", "WORKSPACE", "WORKSPACE.bazel"].iter().copied().collect());

static DEPS_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r#"["']([/@][^"']{1,300})["']"#).unwrap());
static LOAD_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r#"load\(\s*["']([^"']{1,300})["']\s*,"#).unwrap());
static SRCS_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r#"["']([^"']{1,300}\.\w{1,10})["']"#).unwrap());
static LABEL_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r#"//([^:"']{1,200}):([^"'\s,\]]{1,200})"#).unwrap());

fn is_bazel_file(path: &Path) -> bool {
    let name = path.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_default();
    if BAZEL_NAMES.contains(name.as_str()) { return true; }
    let ext = base::file_ext(path);
    ext == ".bzl" || ext == ".bazel"
}

fn extract_labels(content: &str) -> FxHashSet<String> {
    DEPS_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}

fn extract_loads(content: &str) -> FxHashSet<String> {
    LOAD_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}
...
fn extract_srcs(content: &str) -> FxHashSet<String> {
    ...
    for line in content.lines() {
        ...
        if stripped.contains("srcs") && stripped.contains('=') { in_srcs = true; }
        if in_srcs {
            for m in SRCS_RE.captures_iter(line) { srcs.insert(m[1].to_string()); }
            if stripped.contains(']') { in_srcs = false; }
        }
    }
    srcs
}
...
fn label_to_path(label: &str) -> Option<String> {
    ...
    if label.starts_with("//") {
        let cleaned = label.trim_start_matches('/').split(':').next().unwrap_or("");
...
fn ref_to_filename(r: &str) -> String {
    r.trim_end_matches('/').split('/').next_back().unwrap_or(r).split(':').next_back().unwrap_or(r).to_lowercase()
}
...
impl EdgeBuilder for BazelEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_bazel_file(Path::new(f.path()))).collect();
            ...
            for label in extract_labels(&bf.content) {
                if let Some(path) = label_to_path(&label) {
                    for build_name in ["BUILD", "BUILD.bazel"] {
                        link_by_path_match(&bf.id, &format!("{}/{}", path, build_name), &idx, &mut edges, deps_w, rev);
                    }
            ...
            for load in extract_loads(&bf.content) {
                ...
                for (name, frag_ids) in &idx.by_name {
                    if name == &filename || name == stem {
                        for fid in frag_ids {
                            if fid != &bf.id { add_edge(&mut edges, &bf.id, fid, load_w, rev); linked = true; break; }
                        }
                        if linked { break; }
                    }
                }
    ...
    fn discover_related_files(
        ...
        if bazel_changed.is_empty() { return vec![]; }

        let mut refs = FxHashSet::default();
        for f in &bazel_changed {
            let content = match base::read_file_cached(f, file_cache) { Some(c) => c, None => continue };
            ...
            for load in extract_loads(&content) {
                if let Some(path) = label_to_path(&load) { refs.insert(path); }
        ...
        let changed_names: FxHashSet<String> = bazel_changed.iter()
        ...
        for f in &bazel_changed {
            ...
            if base::file_ext(f) == ".bzl" {
                if let Some(stem) = f.file_stem() { changed_paths.insert(stem.to_string_lossy().to_string()); }
            }
        }

        for candidate in candidates {
            if !is_bazel_file(candidate) { continue; }
            if let Some(content) = base::read_file_cached(candidate, file_cache) {
                for load in extract_loads(&content) {
                    let load_file = ref_to_filename(&load);
                    if changed_names.contains(&load_file) || changed_paths.iter().any(|cp| load.contains(cp.as_str())) {
