...
static METHOD_CALL_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(\w+)[:.]\w+\s*\(").unwrap());

fn extract_requires(content: &str) -> FxHashSet<String> {
    let mut refs: FxHashSet<String> = REQUIRE_RE.captures_iter(content).map(|c| c[1].to_string()).collect();
...
fn extract_defs(content: &str) -> FxHashSet<String> {
    ...
    defs.extend(LOCAL_FUNC_RE.captures_iter(content).map(|c| c[1].to_string()));
    defs
}

fn extract_method_targets(content: &str) -> FxHashSet<String> {
    METHOD_CALL_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}
...
impl EdgeBuilder for LuaEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_lua_file(Path::new(f.path()))).collect();
        ...
        for f in &frags {
            ...
            for target in extract_method_targets(&f.content) {
                if self_defs.contains(&target) { continue; }
    ...
    fn discover_related_files(
        ...
        if lua_changed.is_empty() { return vec![]; }
