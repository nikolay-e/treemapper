...
static FIELD_TYPE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*(?:repeated\s+|optional\s+|required\s+|map<\w+,\s*)?([A-Z]\w+)").unwrap());
static RPC_TYPE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?:returns\s*\(|rpc\s+\w+\s*\()\s*(?:stream\s+)?([A-Z]\w+)").unwrap());
...
fn extract_defs(content: &str) -> FxHashSet<String> {
    MSG_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}

fn extract_type_refs(content: &str) -> FxHashSet<String> {
    let mut refs: FxHashSet<String> = FIELD_TYPE_RE.captures_iter(content).map(|c| c[1].to_string()).collect();
...
impl EdgeBuilder for ProtobufEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_proto_file(Path::new(f.path()))).collect();
        ...
        for f in &frags {
            ...
            for tref in extract_type_refs(&f.content) {
                ...
                let w = if RPC_TYPE_RE.captures_iter(&f.content).any(|c| c[1] == tref) { rpc_w } else { msg_w };
    ...
    fn discover_related_files(
        ...
        if proto_changed.is_empty() { return vec![]; }
