...
static IMPORT_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?m)^\s*@(?:import|use|forward)\s+['"]([^'"]+)['"]"#).unwrap()
});
...
fn extract_selectors(content: &str) -> FxHashSet<String> {
    SELECTOR_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}
...
impl EdgeBuilder for CssEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_css_file(Path::new(f.path()))).collect();
    ...
    fn discover_related_files(
        ...
        if css_changed.is_empty() { return vec![]; }
