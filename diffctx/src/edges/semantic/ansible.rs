...
use super::super::base::{self, add_edge, EdgeBuilder, FragmentIndex, discover_files_by_refs, link_by_name, link_by_path_match};
use super::super::EdgeDict;

static ANSIBLE_EXTS: Lazy<FxHashSet<&str>> =
    Lazy::new(|| [".yml", ".yaml", ".j2", ".jinja2", ".jinja"].iter().copied().collect());
...
static ROLES_LIST_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?m)^\s*-\s+(?:role:\s*)?([a-zA-Z_][\w.\-]{0,200})\s*$").unwrap()
});
static VARS_FILES_LIST_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?m)^\s*-\s+["']([^"']{1,300}\.ya?ml)["']"#).unwrap()
});
...
fn ref_to_filename(r: &str) -> String {
    r.trim_end_matches('/').split('/').next_back().unwrap_or(r).to_lowercase()
}

fn extract_refs(content: &str, file_path: &Path) -> FxHashSet<String> {
    let mut refs = FxHashSet::default();
    for m in INCLUDE_VARS_RE.captures_iter(content) { refs.insert(m[1].to_string()); }
    for m in INCLUDE_TASKS_RE.captures_iter(content) { refs.insert(m[1].to_string()); }
    for m in TEMPLATE_SRC_RE.captures_iter(content) { refs.insert(m[1].to_string()); }
    for m in VARS_FILES_LIST_RE.captures_iter(content) { refs.insert(m[1].to_string()); }
...
fn extract_role_refs(content: &str) -> FxHashSet<String> {
    ...
    for line in content.lines() {
        ...
        if stripped.starts_with("roles:") { in_roles = true; continue; }
        if !in_roles { continue; }
        if stripped.starts_with("- ") {
            if let Some(c) = ROLES_LIST_RE.captures(line) { roles.insert(c[1].to_string()); }
...
impl EdgeBuilder for AnsibleEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_ansible_file(Path::new(f.path()))).collect();
    ...
    fn discover_related_files(
        ...
        let ansible_changed: Vec<&PathBuf> = changed.iter().filter(|p| is_ansible_file(p)).collect();
        if ansible_changed.is_empty() { return vec![]; }
        ...
        for f in &ansible_changed {
            if let Some(content) = base::read_file_cached(f, file_cache) {
                ...
                for role in extract_role_refs(&content) { role_names.insert(role); }
            }
            if let Some(role) = get_role_name(f) { role_names.insert(role); }
        }
