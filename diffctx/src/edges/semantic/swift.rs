...
static IMPORT_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*import\s+(?:class|struct|enum|protocol|func\s+)?(\w+)").unwrap());
static TYPE_DEF_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*(?:public|open|internal|private|fileprivate)?\s*(?:final\s+)?(?:class|struct|protocol|enum|actor)\s+([A-Z]\w*)").unwrap());
static FUNC_DEF_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*(?:public|open|internal|private|fileprivate|static|class)?\s*func\s+([a-zA-Z_]\w*)").unwrap());
...
fn extract_type_defs(content: &str) -> FxHashSet<String> {
    let mut defs: FxHashSet<String> = TYPE_DEF_RE.captures_iter(content).map(|c| c[1].to_string()).collect();
...
fn extract_extensions(content: &str) -> FxHashSet<String> {
    EXTENSION_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}
...
impl EdgeBuilder for SwiftEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_swift_file(Path::new(f.path()))).collect();
    ...
    fn discover_related_files(
        ...
        if sw_changed.is_empty() { return vec![]; }
        let mut refs = FxHashSet::default();
        for f in &sw_changed {
