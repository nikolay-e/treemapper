...
static SHORT_FUNC_RE: Lazy<Regex> =
...
fn extract_defs(content: &str) -> FxHashSet<String> {
    let mut defs: FxHashSet<String> = STRUCT_RE.captures_iter(content).map(|c| c[1].to_string()).collect();
    ...
    defs.extend(SHORT_FUNC_RE.captures_iter(content).map(|c| c[1].to_string()));
    defs
}
...
impl EdgeBuilder for JuliaEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_julia_file(Path::new(f.path()))).collect();
        ...
        for f in &frags {
            ...
            for imp in extract_imports(&f.content) {
                let w = if INCLUDE_RE.is_match(&format!("include(\"{}\")", imp)) { include_w } else { using_w };
    ...
    fn discover_related_files(
        ...
        if jl_changed.is_empty() { return vec![]; }
        let mut refs = FxHashSet::default();
        for f in &jl_changed {
