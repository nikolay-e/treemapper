...
static INCLUDE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*include\s+([\w/]+)").unwrap());
static PROC_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*(?:proc|func|method|iterator|converter|template|macro)\s+(\w+)").unwrap());
static TYPE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s*type\s*\n(?:\s+(\w+)\*?\s*=.+\n?)+").unwrap());
static TYPE_SINGLE_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?m)^\s+(\w+)\*?\s*=\s*(?:object|ref|enum|distinct|concept)").unwrap());
static CALL_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(\w+)\s*[\(\[]").unwrap());

static NIM_KEYWORDS: Lazy<FxHashSet<&str>> = Lazy::new(|| {
    ["if", "elif", "else", "when", "case", "of", "for", "while", "block",
     "break", "continue", "return", "result", "proc", "func", "method",
     "var", "let", "const", "type", "import", "from", "include", "export",
     "template", "macro", "iterator", "converter", "object", "ref", "ptr",
     "nil", "true", "false", "and", "or", "not", "xor", "div", "mod",
     "echo", "assert", "doAssert", "len", "add", "del", "new", "newSeq"]
        .iter().copied().collect()
});
...
fn extract_imports(content: &str) -> FxHashSet<String> {
    ...
    refs.extend(INCLUDE_RE.captures_iter(content).map(|c| {
        c[1].split('/').last().unwrap_or(&c[1]).to_string()
    }));
    refs
}

fn extract_defs(content: &str) -> FxHashSet<String> {
    let mut defs: FxHashSet<String> = PROC_RE.captures_iter(content).map(|c| c[1].to_string()).collect();
    defs.extend(TYPE_SINGLE_RE.captures_iter(content).map(|c| c[1].to_string()));
    defs
}
...
impl EdgeBuilder for NimEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_nim_file(Path::new(f.path()))).collect();
        ...
        for f in &frags {
            ...
            for cap in CALL_RE.captures_iter(&f.content) {
                let name = &cap[1];
                if self_defs.contains(name) || NIM_KEYWORDS.contains(name) { continue; }
                let w = if name.starts_with(|c: char| c.is_uppercase()) { type_w } else { fn_w };
    ...
    fn discover_related_files(
        ...
        if nim_changed.is_empty() { return vec![]; }
        let mut refs = FxHashSet::default();
