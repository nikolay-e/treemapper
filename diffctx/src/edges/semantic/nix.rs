...
static FETCH_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?:fetchurl|fetchFromGitHub|fetchgit)\s*\{[^}]*url\s*=\s*['"]([^'"]+)['"]"#).unwrap());
...
fn extract_refs(content: &str) -> FxHashSet<String> {
    ...
    refs.extend(CALL_PACKAGE_RE.captures_iter(content).map(|c| c[1].to_string()));
...
impl EdgeBuilder for NixEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_nix_file(Path::new(f.path()))).collect();
    ...
    fn discover_related_files(
        ...
        if nix_changed.is_empty() { return vec![]; }
        let mut refs = FxHashSet::default();
