...
static R_KEYWORDS: Lazy<FxHashSet<&str>> = Lazy::new(|| {
    ["if", "else", "for", "while", "repeat", "function", "return", "next", "break",
     "in", "TRUE", "FALSE", "NULL", "NA", "Inf", "NaN", "library", "require",
     "source", "print", "cat", "paste", "c", "list", "data.frame", "matrix",
     "length", "nrow", "ncol", "which", "apply", "sapply", "lapply"]
        .iter().copied().collect()
});

fn extract_sources(content: &str) -> FxHashSet<String> {
    SOURCE_RE.captures_iter(content).map(|c| c[1].to_string()).collect()
}

fn extract_defs(content: &str) -> FxHashSet<String> {
    let mut defs: FxHashSet<String> = FUNC_DEF_RE.captures_iter(content).map(|c| c[1].to_string()).collect();
    defs.extend(S4_CLASS_RE.captures_iter(content).map(|c| c[1].to_string()));
    defs.extend(S4_METHOD_RE.captures_iter(content).map(|c| c[1].to_string()));
    defs
}
...
impl EdgeBuilder for RLangEdgeBuilder {
    fn build(&self, fragments: &[Fragment], repo_root: Option<&Path>) -> EdgeDict {
        let frags: Vec<&Fragment> = fragments.iter().filter(|f| is_r_file(Path::new(f.path()))).collect();
        ...
        for f in &frags {
            ...
            for call in extract_calls(&f.content) {
                if self_defs.contains(&call) { continue; }
                let w = if S4_CLASS_RE.is_match(&f.content) || S4_METHOD_RE.is_match(&f.content) { s4_w } else { fn_w };
    ...
    fn discover_related_files(
        ...
        if r_changed.is_empty() { return vec![]; }
