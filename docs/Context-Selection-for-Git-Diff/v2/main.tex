...
\paragraph{Per-source out-edge cap.} The typed dependency graph admits power-law density on large repositories (django, mui/material-ui, microsoft/vscode), with raw edge counts reaching 3--5\,M before any pruning. We cap each source node's outgoing edge list to its top-$K$ neighbors by post-suppression weight. The cap is applied \emph{after} hub-suppression so the IDF damping computes against the full in-degree distribution. We fix $K{=}64$ as a method parameter, not a calibrated hyperparameter, on the basis of two empirical findings on the 21{,}204-instance calibration corpus: (i) within the largest size stratum (graphs with $>$50k raw edges), mean file recall is non-decreasing in cap-firing intensity (recall $0.159, 0.177, 0.186, 0.224$ across cap-drop quartiles $<$25\%, 25--50\%, 50--75\%, $\geq$75\%), so the cap does not depress recall on the instances where it fires aggressively; (ii) a paired per-instance test on $n{=}88$ random instances comparing $K{=}64$ vs.\ $K{=}256$ finds a mean delta of $-0.0047$ ($K{=}256$ slightly worse) with $80/88$ instances unchanged at $\pm 10^{-3}$. We conclude that $K{=}64$ is sufficient on the present corpus and treat the cap as a structural constant rather than a tuning lever; the implementation exposes it via the \texttt{DIFFCTX\_MAX\_EDGES\_PER\_NODE} environment variable for downstream studies.
