...
\subsection{EGO Depth Sweep}
\label{sec:prelim-depth}

To validate the deployed default of $L{=}2$ and the claim in Section~\ref{sec:ego} that ``relevance signal beyond depth 2 is dominated by noise from utility hubs and broad-capture configuration files'', we sweep the EGO traversal radius $L \in \{0, 1, 2, 3, 4\}$ on the full 1500-instance test set at the headline budget $B{=}8000$. Depth $L{=}0$ reduces the scoring stage to seed-only (no graph propagation in the scoring pass; post-passes still run) and serves as a graph-free lower bound for the ablation; depth $L{=}4$ verifies that the $L{=}2$ result is not artificially capped at the deployed default. Table~\ref{tab:prelim-depth} reports the curve.

\begin{table}[h]
\centering
\small
\caption{EGO depth sweep at $B{=}8000$, all 1500 paired instances, with 95\% percentile bootstrap CIs ($B{=}10{,}000$ resamples, seed=42). Pooled and per-test-set columns reported. \emph{Mean used tokens} (last column) is the pooled per-instance mean of selected-fragment token counts under \texttt{tiktoken o200k\_base}; the gap to the $8{,}000$-token nominal budget reflects the lazy-greedy stopping rule. Source: sweep run \texttt{25341657490} at commit \texttt{e869972b} (post Hybrid-removal, post-Lazy-MODE fix). Cells at higher budgets in the same run were lost to a runner-disk fault and are not reported here; the $B{=}8000$ subset of the L grid was complete (5 depths $\times$ 3 datasets = 15 cells).}
\label{tab:prelim-depth}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrllllr}
\toprule
\textbf{Depth $L$} & \textbf{n} & \textbf{ContextBench} & \textbf{PolyBench-500} & \textbf{SWE-bench Verified} & \textbf{Pooled} & \textbf{Used tokens} \\
\midrule
$0$ (seed only) & 1500 & 0.637 [0.604, 0.670] & 0.848 [0.817, 0.877] & 0.913 [0.888, 0.936] & 0.800 [0.781, 0.817] & 1{,}373 \\
$1$            & 1500 & 0.721 [0.690, 0.753] & 0.848 [0.818, 0.877] & 0.913 [0.888, 0.936] & 0.828 [0.811, 0.844] & 3{,}750 \\
$2$ (default)  & 1500 & \textbf{0.731} [0.700, 0.762] & 0.848 [0.817, 0.877] & 0.913 [0.888, 0.936] & 0.831 [0.814, 0.847] & 5{,}098 \\
$3$            & 1500 & 0.732 [0.701, 0.762] & 0.848 [0.818, 0.877] & 0.913 [0.888, 0.936] & 0.831 [0.814, 0.847] & 5{,}189 \\
$4$            & 1500 & 0.735 [0.704, 0.765] & 0.848 [0.817, 0.877] & 0.913 [0.888, 0.936] & 0.832 [0.815, 0.848] & 5{,}188 \\
\bottomrule
\end{tabular}}
\end{table}

The depth curve is informative on three counts. First, on PolyBench-500 and SWE-bench Verified the recall is identical to four decimal places across all five depths; both benchmarks are seed-saturated (the surviving gold files are dominated by the patch's own seeds), so any positive graph propagation reaches the ceiling immediately. Second, on ContextBench Verified — the only benchmark where graph propagation is empirically separable — the curve rises sharply from $L{=}0$ ($0.637$) to $L{=}1$ ($0.721$, $+0.084$), softens to $L{=}2$ ($0.731$, $+0.010$), and is statistically flat from $L{=}2$ through $L{=}4$ (each successive jump is $<0.005$, well inside the bootstrap CI half-width of $\sim 0.030$). Third, mean used tokens grows from $1{,}373$ at $L{=}0$ to $\approx 5{,}100$ at $L{\geq}2$ and saturates: deeper traversal expands the candidate set but does not yield additional content the lazy-greedy selector chooses to keep under the $B{=}8000$ budget. Together these observations validate the deployed $L{=}2$ default empirically: the bulk of the graph signal is captured at radius two, and depths three and four neither add recall nor consume meaningfully more tokens.
