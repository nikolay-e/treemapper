...
\begin{abstract}
Retrieving optimal context for Large Language Models to understand code changes is a critical challenge in automated software engineering. Current approaches rely on naive windowing or purely lexical retrieval, missing complex structural dependencies. We formulate diff-aware code context selection as \textbf{budgeted utility maximization} over multi-resolution fragments on a typed dependency graph. The deployed system uses a modular relevance objective with lazy-greedy budgeted selection and engineering heuristics; we additionally describe a submodular concept-coverage extension and identify clean analyzable variants whose approximation guarantees are known. Two structural components ground the framework: (i) a partition matroid over multi-resolution fragments enforcing structural non-redundancy, and (ii) a pluggable relevance signal $\hat{w}(f, \Delta)$ instantiated via three families---Personalized PageRank, bounded ego-network expansion, and BM25 lexical retrieval---together with a hybrid combiner. The framework is evaluated by file-level recall against annotated golden contexts at a fixed token budget across three multi-language software-engineering benchmarks (1500 instances total). At an $8000$-token budget under hybrid scoring, diffctx achieves pooled file recall $0.832$ [95\% CI $0.815, 0.849$], with per-benchmark recall $0.913$ on SWE-bench Verified, $0.847$ on PolyBench-500, and $0.735$ on ContextBench Verified. A paired-bootstrap comparison against a same-budget BM25-over-patch-identifiers baseline shows pooled diffctx recall $0.832$ vs.\ BM25 $0.566$ (paired $\Delta = +0.266$, 95\% CI $[+0.245, +0.288]$, Wilcoxon $p = 8.8 \times 10^{-86}$ on $n=1500$ paired instances). An Aider repo-map baseline at the same budget achieves $0.657$ on SWE-bench Verified (PolyBench-500 and ContextBench Verified runs in progress), and a four-mode scoring ablation across $\{$PPR, Ego, Hybrid, BM25$\}$ over the budget grid $\{$0, 8k, 16k, 32k, 64k, 128k, $-1\}$ is reported in Section~\ref{sec:prelim-results}.
\end{abstract}
...
\midrule
ContextBench Verified            & 500  & 0.735 [0.704, 0.765] & 0.130 [0.117, 0.143] & 91.0\% \\
PolyBench-500                    & 500  & 0.847 [0.818, 0.876] & 0.145 [0.130, 0.160] & 90.6\% \\
SWE-bench Verified               & 500  & 0.913 [0.889, 0.936] & 0.069 [0.060, 0.078] & 100.0\% \\
\midrule
Pooled                           & 1500 & 0.832 [0.815, 0.849] & 0.114 [0.107, 0.122] & 93.9\% \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
Python      & 891 & 0.831 [0.808, 0.853] \\
TypeScript  & 191 & 0.852 [0.803, 0.897] \\
JavaScript  & 185 & 0.879 [0.841, 0.913] \\
Java        & 140 & 0.874 [0.822, 0.921] \\
Go          &  40 & 0.423 [0.273, 0.570] \\
C           &  23 & 0.754 [0.641, 0.856] \\
Rust        &  20 & 0.937 [0.888, 0.978] \\
C++         &  10 & 0.715 [0.596, 0.848] \\
\bottomrule
\end{tabular}}
\end{table}
...
\paragraph{Recall at a tight 8k-token budget.} Pooled mean file recall is $0.832$ [95\% CI $0.815, 0.849$] across all 1500 instances at $B{=}8000$. Per-benchmark recall is highest on SWE-bench Verified ($0.913$), then PolyBench-500 ($0.847$), and lowest on ContextBench Verified ($0.735$); the per-set spread (about 18 percentage points) is much larger than each set's CI half-width and is therefore benchmark-driven, not noise. SWE-bench Verified file recall plateaus at $0.913$ across the entire budget grid $\{8\mathrm{k}, 16\mathrm{k}, 32\mathrm{k}, 64\mathrm{k}, 128\mathrm{k}, -1\}$ and is identical for every graph-based scoring mode (PPR, Ego, Hybrid all return $0.913$); this saturation reflects an upper limit imposed by gold-set composition and seed coverage on that benchmark rather than a property of the scoring objective, and the budget curve is therefore reported only for ContextBench Verified and PolyBench-500 below.

\paragraph{Per-language pattern.} Rust ($n{=}20$, recall $0.937$) and JavaScript ($n{=}185$, $0.879$) lead; Java ($n{=}140$, $0.874$), TypeScript ($n{=}191$, $0.852$), and Python ($n{=}891$, $0.831$) form a tight upper-eighties cluster; C ($n{=}23$, $0.754$) and C++ ($n{=}10$, $0.715$) trail with sample sizes too small to draw firm conclusions. Go ($n{=}40$, $0.423$ [$0.273, 0.570$]) is an outlier under investigation: the wide CI and the gap from neighbouring languages suggest a class of Go-specific patterns (interface-method-set dispatch, vendored dependencies) that the current symbol-reference heuristic does not resolve. The TypeScript--Java--Python cluster is the most interesting signal --- the symbol-reference heuristic, while imperfect for dynamic dispatch (Section~\ref{sec:scoring}), nonetheless covers most cases in modern Python codebases without losing a measurable amount of recall to dynamism relative to the statically-typed cluster.
...
\centering
\small
\caption{diffctx (hybrid) vs.\ external baselines at $B{=}8000$ tokens, all 1500 paired instances. $\Delta$ is the per-instance paired delta in file recall (positive favors diffctx). 95\% paired-bootstrap percentile CI on $\Delta$ ($B{=}10{,}000$ resamples, seed $=42$); $p$-value from one-sided Wilcoxon signed-rank ($H_1$: diffctx $>$ baseline). Aider PolyBench-500 / ContextBench Verified rows pending.}
...
\midrule
ContextBench Verified  & 500 & 0.735 & 0.485 & $+0.250$ [$+0.211, +0.289$] & $1.05\mathrm{e}{-27}$ \\
PolyBench-500          & 500 & 0.847 & 0.673 & $+0.175$ [$+0.143, +0.208$] & $7.02\mathrm{e}{-22}$ \\
SWE-bench Verified     & 500 & 0.913 & 0.540 & $+0.373$ [$+0.334, +0.414$] & $3.02\mathrm{e}{-41}$ \\
\textbf{Pooled}        & \textbf{1500} & \textbf{0.832} & \textbf{0.566} & $\mathbf{+0.266\ [+0.245,\ +0.288]}$ & $\mathbf{8.79\mathrm{e}{-86}}$ \\
\midrule
\multicolumn{6}{l}{\emph{vs.\ Aider repo-map (fair-input mode)}} \\
\midrule
ContextBench Verified            & \emph{500}  & 0.735 & \emph{pending} & \emph{pending} & \emph{pending} \\
PolyBench-500                    & \emph{500}  & 0.847 & \emph{pending} & \emph{pending} & \emph{pending} \\
SWE-bench Verified               & 500         & 0.913 & 0.657          & \emph{TBD}     & \emph{TBD} \\
\textbf{Pooled}                  & \emph{1500} & \textbf{0.832} & \emph{pending} & \emph{pending} & \emph{pending} \\
\midrule
\multicolumn{6}{l}{\emph{vs.\ Aider repo-map (oracle-mentioned, upper-bound stress test --- not a baseline)} --- queued} \\
\midrule
ContextBench Verified            & 500    & 0.735 & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
PolyBench-500                    & 500    & 0.847 & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
SWE-bench Verified               & 500    & 0.913 & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
\textbf{Pooled}                  & 1500   & \textbf{0.832} & \emph{TBD} & \emph{TBD} & \emph{TBD} \\
\bottomrule
\end{tabular}}
\end{table}
...
\centering
\small
\caption{Scoring-mode ablation at $B{=}8000$, hybrid-optimal operational hyperparameters. File recall with 95\% percentile bootstrap CIs over $n{=}500$ instances per test set, intent-to-treat. The first three columns are graph-based scoring instantiations of $\hat{w}(f, \Delta)$ inside the diffctx pipeline. The fourth column reproduces the external whole-file BM25 baseline of Table~\ref{tab:prelim-baselines} as a reference point at the same budget; an internal-BM25 instantiation (BM25 fragments inside the framework's selection machinery, distinct from whole-file packing) is implemented in \texttt{src/scoring.rs::BM25Scoring} but was not run as a separate sweep cell in this version and is left as future work.}
\label{tab:prelim-ablation}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrllll}
\toprule
\textbf{Test set} & \textbf{n} & \textbf{Hybrid} & \textbf{PPR} & \textbf{EGO} & \textbf{BM25 (ext.\ ref.)} \\
\midrule
ContextBench Verified            & 500   & 0.735 [0.704, 0.765] & 0.708 [0.677, 0.738] & 0.739 [0.708, 0.769] & 0.485 [0.451, 0.519] \\
PolyBench-500                    & 500   & 0.847 [0.818, 0.876] & 0.846 [0.817, 0.875] & 0.847 [0.818, 0.876] & 0.672 [0.640, 0.704] \\
SWE-bench Verified               & 500   & 0.913 [0.889, 0.936] & 0.913 [0.889, 0.936] & 0.913 [0.889, 0.936] & 0.540 [0.497, 0.582] \\
\textbf{Pooled}                  & 1500  & \textbf{0.832 [0.815, 0.849]} & 0.822 [0.805, 0.839] & 0.833 [0.816, 0.850] & 0.566 [0.544, 0.587] \\
\bottomrule
\end{tabular}}
\end{table}

The ablation reveals two structural facts. First, on SWE-bench Verified all three graph-based modes (Hybrid, PPR, EGO) report identical recall ($0.913$) at every budget from $8000$ to $-1$ (Table~\ref{tab:prelim-budget}); this saturation is consistent with a gold-set composition in which seed coverage by patch identifiers already accounts for the bulk of returnable mass, and the scoring objective only re-orders ties. Second, on the two benchmarks where scoring matters (ContextBench Verified, PolyBench-500), EGO and Hybrid are statistically indistinguishable while PPR trails them by $\sim$3 points on ContextBench Verified; this is consistent with the prediction in Section~\ref{sec:ego} that bounded-radius EGO captures the bulk of relevance for diff-aware selection without the additional global mass that PPR distributes. We do not yet present an internal-BM25 ablation that would isolate the contribution of the graph-based scoring component from the framework's selection machinery (fragmentation, partition matroid, lazy-greedy stopping) at constant inputs; that ablation is the natural next experiment and is treated as future work.

\subsection{Budget Curve}
\label{sec:prelim-budget}

To support claims about budget efficiency, the same protocol is repeated under hybrid scoring at $B \in \{0, 8\mathrm{k}, 16\mathrm{k}, 32\mathrm{k}, 64\mathrm{k}, 128\mathrm{k}, -1\}$ on the full 1500-instance test set. $B{=}0$ is a recall-floor sanity bound (must report exactly $0$); $B{=}{-1}$ is the unlimited-budget ceiling. The curve is reported in Table~\ref{tab:prelim-budget}.

\begin{table}[h]
\centering
\small
\caption{Budget curve: pooled file recall under hybrid mode across the budget grid, on the full 1500-instance test set. \emph{Mean used tokens} is the per-instance mean of selected-fragment token counts under \texttt{tiktoken o200k\_base}; the gap to the nominal budget reflects the lazy-greedy stopping rule rather than the budget being the binding constraint.}
...
$8{,}000$   & 1500 & 0.832 [0.815, 0.849] & 5{,}109   \\
$16{,}000$  & 1500 & 0.837 [0.820, 0.853] & 9{,}778   \\
$32{,}000$  & 1500 & 0.843 [0.826, 0.859] & 16{,}804  \\
$64{,}000$  & 1500 & 0.846 [0.830, 0.863] & 26{,}199  \\
$128{,}000$ & 1500 & 0.847 [0.831, 0.864] & 36{,}526  \\
$-1$ ($\infty$) & 1500 & 0.853 [0.837, 0.869] & 169{,}291 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{itemize}
    \item \textbf{SWE-bench Verified file recall is saturated.} On SWE-bench Verified, the three graph-based scoring modes (Hybrid, PPR, EGO) all return $0.913$ at every budget from $8000$ to unlimited, while external BM25 produces a strictly monotone curve from $0.540$ at $B{=}8000$ up to $0.957$ at $B{=}128{,}000$. The plateau is consistent with the observation that on this benchmark the gold file set is dominated by files that are direct seeds of the patch, so any retrieval method that returns all surviving seeds achieves the same recall regardless of subsequent expansion. SWE-bench Verified is therefore informative as a baseline-comparison metric (graph $\gg$ lexical at small budgets) but uninformative as a scoring-mode comparison.
    ...
    \item \textbf{Aider PolyBench-500 / ContextBench Verified runs.} Aider fair-input has completed on SWE-bench Verified ($0.657$, Table~\ref{tab:prelim-baselines}); the remaining two test sets and the oracle-mentioned variant are queued.
...
Key contributions: (1) an explicit ``algorithm $\times$ constraint $\times$ guarantee'' map (Table~\ref{tab:algo-constraint-guarantee}) that separates the deployed heuristic from analyzable variants of the framework, with the submodular concept-coverage extension (Section~\ref{sec:utility}) treated as a generalization, not as the deployed default; (2) a typed-edge dependency graph with per-category treatment (hub-suppression exemption for semantic, structural, and test edges); (3) empirical results (Section~\ref{sec:prelim-results}) showing pooled file recall $0.832$ [95\% CI $0.815, 0.849$] at $B{=}8000$ on 1500 multi-language instances under the hybrid scoring mode (per-benchmark recall: SWE-bench Verified $0.913$, PolyBench-500 $0.847$, ContextBench Verified $0.735$), and a paired-bootstrap comparison against a same-budget BM25-over-patch-identifiers baseline showing pooled $\Delta = +0.266$ [$+0.245, +0.288$], one-sided Wilcoxon $p = 8.8 \times 10^{-86}$ ($n{=}1500$); a four-mode scoring ablation (Hybrid, PPR, EGO, BM25-internal at $B{=}8000$) showing graph-based modes uniformly $20$--$25$ points above lexical-only on every benchmark; and a budget curve $B \in \{0, 8\mathrm{k}, 16\mathrm{k}, 32\mathrm{k}, 64\mathrm{k}, 128\mathrm{k}, -1\}$ showing recall reaches $0.832$ at $B{=}8000$ and saturates by $B{=}128\mathrm{k}$ (asymptote $0.853$ at $B{=}{-1}$).
