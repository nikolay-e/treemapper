...
\centering
\small
\caption{Per-benchmark file-level metrics, scoring=hybrid, $B{=}8000$ tokens, with 95\% percentile bootstrap CIs ($B{=}10{,}000$ resamples, seed=42). Status \texttt{ok} excludes \texttt{clone\_fail} and execution errors. Macro and micro averages coincide because each test set has $n{=}500$.}
...
\centering
\small
\caption{Scoring-mode ablation at $B{=}8000$, hybrid-optimal operational hyperparameters. File recall with 95\% percentile bootstrap CIs. \emph{BM25 (internal)} is the BM25 instantiation of $\hat{w}(f, \Delta)$ inside the diffctx pipeline (uses the framework's fragmentation, partition matroid, and lazy-greedy budgeted selection); it is distinct from the external file-level BM25 baseline of Table~\ref{tab:prelim-baselines} which packs whole files in BM25 rank order and is therefore not directly comparable.}
