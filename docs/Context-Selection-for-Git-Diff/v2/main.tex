...
\title{diffctx: A Submodular Framework for Diff-Aware Code Context Selection}
...
\begin{enumerate}
    ...
    \item A pluggable relevance-scoring abstraction $\hat{w}(f, \Delta)$ where multiple algorithms (Personalized PageRank, bounded ego-network expansion, BM25) instantiate the abstract signal under a single submodular framework, with a hybrid combiner adapting among them at query time
...
\begin{itemize}
    ...
    \item \textbf{Aider repo-map}~\cite{aider2023repomap} (\texttt{benchmarks/baselines/aider\_baseline.py}): the closest publicly available approach using the same primitives (tree-sitter AST extraction + PageRank + token-budgeted selection). Aider is invoked with \texttt{mentioned\_fnames} populated from the gold patch and \texttt{map\_tokens}=$B$, an asymmetry favoring Aider; diffctx receives only the patch text.
\end{itemize}
...
We have presented diffctx, a framework for diff-aware code context selection that casts the problem as budgeted submodular maximization on a typed dependency graph. The formulation unifies structural and lexical signals under a single submodular objective, leverages well-studied lazy-greedy optimization techniques, and admits an adaptive stopping heuristic based on marginal utility. A pluggable scoring abstraction (PPR, EGO, BM25, Hybrid) allows the same selection algorithm to be instantiated with different relevance signals without changing the optimization machinery.

Key contributions: (1) submodularity of the max-coverage utility under nondecreasing aggregation, with explicit constant-factor approximation guarantees ($\frac{1}{2}(1-1/e)$ via modified-greedy on the knapsack form, $(1-1/e)$ via Boltzmann reweighting on the matroid form); (2) a typed-edge dependency graph with per-category treatment (hub-suppression exemption for semantic, structural, and test edges); (3) interim empirical results (Section~\ref{sec:prelim-results}) showing pooled file recall $0.855$ at $B{=}8000$ on 845 multi-language instances under the hybrid scoring mode, with the rest of the protocol --- baseline comparisons, scoring-mode ablation, budget curve --- scheduled and tracked.
