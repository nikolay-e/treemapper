...
\subsubsection{File-Importance Prior for Impact Needs}
\label{sec:importance}

For \emph{impact-type} needs (callers and dependents of a modified symbol) the match strength is scaled by a per-fragment importance factor $I(f) \in (0, 1]$:
\begin{equation}
m'(f, n) = m(f, n) \cdot I(f), \qquad n.\mathrm{type} = \mathrm{impact}.
\end{equation}
The motivation is that production callers (e.g.\ \texttt{handler.ts}) and peripheral usages (e.g.\ \texttt{examples/parsing.ts}) reference the same symbol but carry different review value; without $I(f)$ they receive identical match strength. Other need types (definition, signature, test, invariant, background) use unmodified $m(f, n)$ since they admit no analogous production-vs-peripheral distinction.

We construct $I(f)$ from path conventions:
\begin{equation}
I(f) = \begin{cases}
0.10 & \text{if } f \text{ resides in a generated path (e.g.\ \texttt{generated/}, \texttt{\_\_generated\_\_/}})\\
0.15 & \text{if } f \text{ matches peripheral patterns (e.g.\ \texttt{examples/}, \texttt{demo/}, \texttt{vendor/}, \texttt{fixtures/}, \texttt{docs/}, \texttt{tutorials/})}\\
1.0 & \text{otherwise}
\end{cases}
\end{equation}
The values are fixed from domain priors and not calibrated to the benchmark, deliberately avoiding overfitting feature engineering to a specific evaluation set. The order-of-magnitude argument is: the cap must be $\ll 1$ (otherwise peripheral matches dominate and the prior fails to discriminate) and $> 0$ (otherwise schema-impact signal through generated code is lost entirely). Generated code is weighted slightly below peripheral because it is mechanically derived from a schema and carries no human-intent signal, whereas peripheral code is human-authored and may convey intended-usage patterns. Sensitivity to $\pm 25\%$ and $\pm 50\%$ perturbation should be checked empirically before reporting headline numbers.

Since $I(f)$ is a per-fragment static property of the path---independent of the selection $C$---and lies in $(0, 1]$, $m'(f, n) \leq m(f, n)$ pointwise, preserving the submodularity argument of the theorem above: scaling a non-negative input to $\phi(\max \cdot)$ by a constant in $(0, 1]$ retains monotonicity and submodularity.
